...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuffALT:Array<Dynamic> = [
		['Z', 0.2], //From 0% to 19%
		['F', 0.4], //From 20% to 39%
		['E', 0.5], //From 40% to 49%
		['D', 0.6], //From 50% to 59%
	...
	override public function create()
	{
		...
		scoreTxt.setFormat(Paths.font("cool.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botplayTxt.setFormat(Paths.font("bot.ttf"), 32, FlxColor.PURPLE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		#else
		botplayTxt = new FlxText(400, healthBar.y - 90, FlxG.width - 800, Language.getPhrase("CHEATER!").toUpperCase(), 32);
	...
	public dynamic function updateScoreText()
	{
		...
		var rank:String = Language.getPhrase('rating_$rankName', rankName);
		if(totalPlayed != 0)
		{
		...
		if (!instakillOnMiss)
			tempScore = Language.getPhrase('score_text', 'Score: {1} | Nope: {2} | Results: {3} | Rank: {4} ({5})', [songScore, songMisses, str, rank, fc]);
		else
			tempScore = Language.getPhrase('score_text_instakill', 'Score: {1} | Results: {2} | Rank: {3} ({4})', [songScore, str, rank, fc]);
...
override public function update(elapsed:Float)
{
	...
	#if developer_mode
	callOnScripts('hideHud', !ClientPrefs.data.hideHud);
	callOnScripts('onDevOnly', !ClientPrefs.data.devMode);
	setOnScripts('DevYes', !ClientPrefs.data.devMode);
	#end
...
public function RecalculateRating(badHit:Bool = false, scoreBop:Bool = true)
{
	...
	if (ret != LuaUtils.Function_Stop)
	{
		...
		if (totalPlayed != 0) // Prevent divide by 0
		{
			...
			// Ranking Name
			rankName = ratingStuffALT[ratingStuffALT.length - 1][0]; // Uses last string
			if (ratingPercent < 1)
				for (i in 0...ratingStuffALT.length - 1)
					if (ratingPercent < ratingStuffALT[i][1])
					{
						rankName = ratingStuffALT[i][0];
						break;
					}
		}
		fullComboFunction();
	...
	for (name in achievesToCheck)
	{
		...
		if (name != WeekData.getWeekFileName() + '_nomiss') // common achievements
		{
				switch(name)
				{
					...
					case 'special_1':
						unlock = (songName == 'win-dad-battle-(sp)' && isStoryMode && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice);
					
					case 'special_2':
						unlock = (songName == 'og-(sp)' && isStoryMode && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice);

					case 'special_3':
						unlock = (songName == 'boogie-(sp)' && isStoryMode && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice);

					case 'special_4':
						unlock = (songName == 'dlow-(sp)' && isStoryMode && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice);

					case 'special_5':
						unlock = (songName == 'untold-loneliness-(sp)' && isStoryMode && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice);

					case 'special_6':
						unlock = (songName == 'careless-(sp)' && isStoryMode && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice);

					case 'special_7':
						unlock = (songName == 'remembrance-final-mix-(sp)' && isStoryMode && storyPlaylist.length <= 1 && !usedPractice);

					case 'special_8':
						unlock = (songName == 'win-milf-(sp)' && isStoryMode && storyPlaylist.length <= 1 && !usedPractice);
